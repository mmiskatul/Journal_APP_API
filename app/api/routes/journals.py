...
def get_journal(entry_id: str, db=Depends(get_db), current_user=Depends(get_current_active_user)):
...
def update_journal(
    ...
    return journal_service.update_entry(db, entry_id, current_user, payload.model_dump())


@router.delete("/{entry_id}")
def delete_journal(entry_id: str, db=Depends(get_db), current_user=Depends(get_current_active_user)):
    journal_service.delete_entry(db, entry_id, current_user)
